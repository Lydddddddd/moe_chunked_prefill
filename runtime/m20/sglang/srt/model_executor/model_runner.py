    ...
    def _forward_raw(
        ...
        try:
            from sglang.srt.layers.moe.kt_ep_wrapper import (
                set_runtime_forward_context_from_forward_batch,
            )

            set_runtime_forward_context_from_forward_batch(forward_batch)
        except Exception:
            pass
        ...
        if forward_batch.forward_mode.is_decode():
            ...
            if getattr(self.server_args, "kt_group_expert_buffer", False):
                from sglang.srt.layers.moe.kt_ep_wrapper import (
                    abort_kt_group_forward,
                    activate_kt_group,
                    begin_kt_group_action,
                    begin_kt_group_forward,
                    end_kt_group_action,
                    end_kt_group_forward,
                    finish_kt_group,
                    get_kt_group_expert_buffer_manager,
                    prefetch_kt_group_action,
                )
                ...
                group_id = int(forward_batch.split_index) // manager.config.group_size
                start, end = manager.config.group_range(group_id)
                if int(forward_batch.split_index) != start:
                    raise RuntimeError(
                        "KT group split-prefill cursor is not at a group boundary: "
                        f"cursor={forward_batch.split_index} expected={start}"
                    )
                if int(split_forward_count) != end - start:
                    raise RuntimeError(
                        "KT scheduler/group size mismatch: "
                        f"forward_count={split_forward_count} expected={end - start}"
                    )
                try:
                    stage_ready = bool(
                        getattr(self.server_args, "kt_stage_ready_scheduler", False)
                    )
                    if stage_ready:
                        if manager.step_active:
                            raise RuntimeError(
                                "KT group action is already active before dispatch"
                            )
                        num_tokens = int(
                            getattr(forward_batch, "extend_num_tokens", 0)
                            or forward_batch.input_ids.numel()
                        )
                        action_rows = []
                        next_action_rows = []
                        for metadata in forward_batch.kt_metadata_list or []:
                            action = (
                                metadata.get("_kt_stage_action")
                                if isinstance(metadata, dict)
                                else None
                            )
                            if not isinstance(action, dict):
                                raise RuntimeError(
                                    "stage ForwardBatch row has no action ticket metadata"
                                )
                            action_rows.append(action)
                            next_action_rows.append(
                                metadata.get("_kt_stage_next_action")
                                if isinstance(metadata, dict)
                                else None
                            )
                        if len(action_rows) != int(forward_batch.batch_size):
                            raise RuntimeError(
                                "stage action metadata count does not match batch size"
                            )
                        action = action_rows[0]
                        if any(row != action for row in action_rows[1:]):
                            raise RuntimeError(
                                "packed stage cohort contains different action tickets"
                            )
                        next_action = next_action_rows[0]
                        if any(row != next_action for row in next_action_rows[1:]):
                            raise RuntimeError(
                                "packed stage cohort contains different next-action hints"
                            )
                        if next_action is not None and not isinstance(next_action, dict):
                            raise RuntimeError("stage next-action hint is not a dictionary")
                        if int(action.get("group_id", -1)) != group_id:
                            raise RuntimeError(
                                "stage action ticket group does not match split cursor"
                            )
                        normalized_plans = normalize_layer_plans(
                            (int(layer_idx), experts)
                            for layer_idx, experts in action.get("layer_plans", [])
                        )
                        expected_hash = compute_plan_hash(group_id, normalized_plans)
                        if str(action.get("plan_hash", "")) != expected_hash:
                            raise RuntimeError("stage action ticket plan hash mismatch")
                        explicit_plans = (
                            dict(normalized_plans) if normalized_plans else None
                        )
                        normalized_source_plans = normalize_layer_plans(
                            (int(layer_idx), experts)
                            for layer_idx, experts in action.get(
                                "logical_source_plans", []
                            )
                        )
                        logical_source_plans = (
                            dict(normalized_source_plans)
                            if normalized_source_plans
                            else None
                        )
                        manager, _action_plans = begin_kt_group_action(
                            group_id,
                            num_tokens,
                            plans=explicit_plans,
                            ticket_id=int(action["ticket_id"]),
                            logical_source_plans=logical_source_plans,
                            logical_source_buffer_id=action.get(
                                "logical_source_buffer_id"
                            ),
                            plan_hash=(
                                str(action["plan_hash"])
                                if explicit_plans is not None
                                else None
                            ),
                        )
                        materialized_plans = tuple(
                            (layer_idx, tuple(experts))
                            for layer_idx, experts in sorted(_action_plans.items())
                        )
                    elif group_id == 0:
                        if manager.step_active:
                            raise RuntimeError("KT group step is already active at layer 0")
                        num_tokens = int(
                            getattr(forward_batch, "extend_num_tokens", 0)
                            or forward_batch.input_ids.numel()
                        )
                        manager = begin_kt_group_forward(num_tokens)
                        if manager.config.num_layers != self.model_config.num_hidden_layers:
                            raise RuntimeError(
                                "KT group manager/model layer mismatch: "
                                f"{manager.config.num_layers} != "
                                f"{self.model_config.num_hidden_layers}"
                            )
                    elif not manager.step_active:
                        raise RuntimeError("KT group step disappeared before final layer")
                    activate_kt_group(group_id)
                    if stage_ready:
                        materialization = manager.action_materialization_snapshot()
                        forward_batch._kt_stage_materialized_action = {
                            "ticket_id": int(action["ticket_id"]),
                            "group_id": group_id,
                            "layer_plans": [
                                [layer_idx, list(experts)]
                                for layer_idx, experts in materialized_plans
                            ],
                            "plan_hash": compute_plan_hash(
                                group_id, materialized_plans
                            ),
                            "target_buffer_id": materialization.get(
                                "target_buffer_id"
                            ),
                            "expected_buffer_versions": materialization.get(
                                "expected_buffer_versions", [-1, -1]
                            ),
                            "copy_ops": materialization.get("copy_ops", []),
                            "materialization": materialization.get(
                                "materialization", "full"
                            ),
                            "changed_by_layer": materialization.get(
                                "changed_by_layer", {}
                            ),
                            "max_changed": materialization.get("max_changed", 0),
                            "h2d_experts": materialization.get("h2d_experts", 0),
                            "d2d_experts": materialization.get("d2d_experts", 0),
                            "retained_experts": materialization.get(
                                "retained_experts", 0
                            ),
                            "h2d_bytes": materialization.get("h2d_bytes", 0),
                            "d2d_bytes": materialization.get("d2d_bytes", 0),
                            "retained_bytes": materialization.get(
                                "retained_bytes", 0
                            ),
                        }
                        if next_action is not None:
                            prefetch_kt_group_action(next_action)
                    if not stage_ready:
                        manager.submit_host_prefetch_for_next_group(group_id)
                        manager.enqueue_next_group_h2d_after_host_prefetch(group_id)
                    ret = self.forward_split_prefill(
                        forward_batch,
                        reinit_attn_backend=(reinit_attn_backend or stage_ready),
                        forward_count=split_forward_count,
                    )
                    finish_kt_group(group_id)
                    if stage_ready:
                        end_kt_group_action(group_id)
                    elif group_id == manager.config.num_groups - 1:
                        end_kt_group_forward()
                except Exception as exc:
                    abort_kt_group_forward(f"{type(exc).__name__}: {exc}")
                    raise
            else:
                ret = self.forward_split_prefill(
                    forward_batch,
