...
class ServerArgs:
    ...
    kt_split_prefill_group_size: int = 0
    ...
    kt_group_oracle_trace: Optional[str] = None
    kt_group_oracle_prompt_identity_manifest: Optional[str] = None
    kt_stage_ready_scheduler: bool = False
    kt_stage_policy: str = "fifo"
    kt_stage_cohort_size: int = 1
    kt_stage_candidate_window: int = 1
    kt_stage_max_consecutive: int = 2
    kt_stage_max_wait_ms: float = 0.0
    kt_stage_max_inflight_chunks: int = 8
    kt_stage_h2d_expert_ms: float = 5.4
    kt_stage_d2d_expert_ms: float = 0.08
    kt_stage_route_entry_gain_ms: float = 0.0
    kt_stage_copy_contention_ms_per_expert: float = 0.0
    kt_stage_eviction_route_weight: float = 0.0
    kt_stage_queue_penalty_ms_per_s: float = 0.0
    kt_stage_min_gain_ms: float = 0.0
    kt_stage_confidence_threshold: float = 1.0
    kt_action_trace_path: Optional[str] = None
    kt_action_replay_path: Optional[str] = None
    ...
    def __post_init__(self):
        ...
        if self.kt_split_prefill_group_size < 0:
            raise ValueError("--kt-split-prefill-group-size cannot be negative")
        if self.kt_split_prefill_group_size > 0:
            if self.enable_pdmux:
                raise ValueError("KT FIFO split prefill is incompatible with PDMux")
            if not self.disable_overlap_schedule:
                logger.warning(
                    "Overlap scheduling is disabled for KT FIFO split prefill"
                )
                self.disable_overlap_schedule = True
            if not self.disable_cuda_graph:
                logger.warning("CUDA graph is disabled for KT FIFO split prefill")
                self.disable_cuda_graph = True
            self.enable_piecewise_cuda_graph = False
        self._handle_kt_group_expert_buffer()
        self._handle_kt_stage_ready_scheduler()
    ...
    def _handle_kt_group_expert_buffer(self):
        if not self.kt_group_expert_buffer:
            return
        if self.kt_weight_path is None:
            raise ValueError("--kt-group-expert-buffer requires --kt-weight-path")
        if str(self.kt_method).upper() != "LLAMAFILE":
            raise ValueError(
                "M20-A group expert buffers currently require --kt-method LLAMAFILE"
            )
        if self.tp_size != 1 or self.pp_size != 1:
            raise ValueError("M20-A group expert buffers currently require TP=1 and PP=1")
        if self.kt_group_buffer_count != 2:
            raise ValueError(
                "M20-A is a double-buffer implementation; "
                "--kt-group-buffer-count must be 2"
            )
        if self.kt_group_size <= 0:
            raise ValueError("--kt-group-size must be positive")
        if self.kt_slots_per_layer <= 0:
            raise ValueError("--kt-slots-per-layer must be positive")
        if self.kt_group_load_mode not in {"sync", "async"}:
            raise ValueError("--kt-group-load-mode must be sync or async")
        if self.kt_group_miss_policy not in {"block", "cpu_fallback"}:
            raise ValueError(
                "--kt-group-miss-policy must be block or cpu_fallback"
            )
        if self.kt_group_prefetch_policy not in {"oracle", "static"}:
            raise ValueError("--kt-group-prefetch-policy must be oracle or static")
        if (
            self.kt_group_load_mode == "sync"
            and self.kt_group_miss_policy == "cpu_fallback"
        ):
            raise ValueError("cpu_fallback is meaningful only with async group loading")
        if self.kt_gpu_experts_ratio is not None:
            raise ValueError(
                "--kt-gpu-experts-ratio is incompatible with group-buffer mode; "
                "use --kt-slots-per-layer"
            )
        if self.kt_group_materialization not in {"full", "delta"}:
            raise ValueError(
                "--kt-group-materialization must be 'full' or 'delta'"
            )
        if self.kt_group_max_replacements is None:
            self.kt_group_max_replacements = self.kt_slots_per_layer
        if not 0 <= self.kt_group_max_replacements <= self.kt_slots_per_layer:
            raise ValueError(
                "--kt-group-max-replacements must be in "
                f"[0, {self.kt_slots_per_layer}]"
            )
        if (
            self.kt_num_gpu_experts is not None
            and self.kt_num_gpu_experts != self.kt_slots_per_layer
        ):
            raise ValueError(
                "--kt-num-gpu-experts must be omitted or equal "
                "--kt-slots-per-layer in group-buffer mode"
            )
        if self.kt_enable_dynamic_expert_update:
            raise ValueError(
                "--kt-enable-dynamic-expert-update is incompatible with "
                "--kt-group-expert-buffer"
            )
        if self.kt_lora_path or self.kt_expert_lora_path:
            raise ValueError("M20-A group expert buffers do not support KT LoRA")
        if self.enable_pdmux:
            raise ValueError(
                "M20-A uses its own FIFO split-prefill loop and cannot enable PDMux"
            )
        if self.kt_group_prefetch_policy == "oracle":
            trace = self.kt_group_oracle_trace or os.environ.get(
                "SGLANG_KT_RUNTIME_ORACLE_PREFETCH_TRACE"
            )
            if self.kt_group_oracle_required and not trace:
                raise ValueError(
                    "oracle group prefetch requires --kt-group-oracle-trace"
                )
        self.kt_num_gpu_experts = self.kt_slots_per_layer
        self.kt_gpu_prefill_token_threshold = 0
        self.kt_split_prefill_group_size = self.kt_group_size
        self.disable_overlap_schedule = True
        if not self.disable_cuda_graph:
            logger.warning("CUDA graph is disabled for KT group expert buffers")
            self.disable_cuda_graph = True
        self.enable_piecewise_cuda_graph = False

    def _handle_kt_stage_ready_scheduler(self):
        if not self.kt_stage_ready_scheduler:
            if self.kt_action_trace_path or self.kt_action_replay_path:
                raise ValueError(
                    "--kt-action-trace-path/--kt-action-replay-path require "
                    "--kt-stage-ready-scheduler"
                )
            return
        if not self.kt_group_expert_buffer:
            raise ValueError(
                "--kt-stage-ready-scheduler requires --kt-group-expert-buffer"
            )
        if self.kt_stage_policy not in {"fifo", "min_delta", "cost_oracle"}:
            raise ValueError(
                "--kt-stage-policy must be fifo, min_delta, or cost_oracle"
            )
        if (
            self.kt_group_load_mode == "async"
            and not self.kt_action_replay_path
        ):
            raise ValueError(
                "ticket-aware async stage materialization requires "
                "--kt-action-replay-path so current and lookahead action order is frozen"
            )
        if (
            self.kt_group_load_mode == "async"
            and self.kt_group_miss_policy != "block"
        ):
            raise ValueError(
                "ticket-aware async stage materialization currently requires "
                "--kt-group-miss-policy block"
            )
        if self.kt_stage_cohort_size <= 0:
            raise ValueError("--kt-stage-cohort-size must be positive")
        if self.kt_stage_candidate_window < self.kt_stage_cohort_size:
            raise ValueError(
                "--kt-stage-candidate-window must be at least "
                "--kt-stage-cohort-size"
            )
        if self.kt_stage_max_consecutive <= 0:
            raise ValueError("--kt-stage-max-consecutive must be positive")
        if self.kt_stage_max_wait_ms < 0:
            raise ValueError("--kt-stage-max-wait-ms cannot be negative")
        if self.kt_stage_max_inflight_chunks <= 0:
            raise ValueError("--kt-stage-max-inflight-chunks must be positive")
        nonnegative_costs = {
            "--kt-stage-h2d-expert-ms": self.kt_stage_h2d_expert_ms,
            "--kt-stage-d2d-expert-ms": self.kt_stage_d2d_expert_ms,
            "--kt-stage-route-entry-gain-ms": self.kt_stage_route_entry_gain_ms,
            "--kt-stage-copy-contention-ms-per-expert": (
                self.kt_stage_copy_contention_ms_per_expert
            ),
            "--kt-stage-eviction-route-weight": self.kt_stage_eviction_route_weight,
            "--kt-stage-queue-penalty-ms-per-s": (
                self.kt_stage_queue_penalty_ms_per_s
            ),
        }
        if any(
            not math.isfinite(value) or value < 0
            for value in nonnegative_costs.values()
        ):
            raise ValueError(
                f"M20-B cost parameters must be non-negative: {nonnegative_costs}"
            )
        if not 0.0 <= self.kt_stage_confidence_threshold <= 1.0:
            raise ValueError("--kt-stage-confidence-threshold must be in [0, 1]")
        if not math.isfinite(self.kt_stage_min_gain_ms):
            raise ValueError("--kt-stage-min-gain-ms must be finite")
        if self.kt_action_trace_path and self.kt_action_replay_path:
            raise ValueError(
                "--kt-action-trace-path and --kt-action-replay-path are mutually exclusive"
            )
        if self.prefill_max_requests not in {None, 1}:
            raise ValueError(
                "M20-B stage states require --prefill-max-requests 1 or omitted"
            )
        self.prefill_max_requests = 1
        if self.enable_mixed_chunk:
            logger.warning("Mixed chunking is disabled for M20-B stage ownership")
    ...
    def _handle_a2a_moe(self):
        ...
        if self.kt_weight_path is not None and not self.disable_overlap_schedule:
            self.disable_overlap_schedule = True
            logger.warning(
                "KTransformers EP is enabled. --disable-overlap-schedule is "
                "automatically set because KT CPU staging buffers do not support "
                "multiple model forwards in flight."
            )
    ...
    def add_cli_args(parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            "--kt-split-prefill-group-size",
            type=int,
            default=ServerArgs.kt_split_prefill_group_size,
            help="[experimental KT] FIFO split-prefill layer count without physical group buffers.",
        )
        parser.add_argument(
            "--kt-group-expert-buffer",
            action="store_true",
            default=ServerArgs.kt_group_expert_buffer,
            help="[experimental KT] Use a physically bounded group expert double buffer.",
        )
        parser.add_argument(
            "--kt-group-size",
            type=int,
            default=ServerArgs.kt_group_size,
            help="[experimental KT] Number of consecutive MoE layers per group.",
        )
        parser.add_argument(
            "--kt-slots-per-layer",
            type=int,
            default=ServerArgs.kt_slots_per_layer,
            help="[experimental KT] Resident expert slots for each layer in a group.",
        )
        parser.add_argument(
            "--kt-group-buffer-count",
            type=int,
            default=ServerArgs.kt_group_buffer_count,
            help="[experimental KT] Physical group buffer count; M20-A requires 2.",
        )
        parser.add_argument(
            "--kt-group-load-mode",
            choices=["sync", "async"],
            default=ServerArgs.kt_group_load_mode,
            help=(
                "[experimental KT] Load synchronously, or pipeline the next frozen "
                "action-replay ticket into the inactive group buffer."
            ),
        )
        parser.add_argument(
            "--kt-group-miss-policy",
            choices=["block", "cpu_fallback"],
            default=ServerArgs.kt_group_miss_policy,
            help="[experimental KT] Action when an asynchronously loaded group is not ready.",
        )
        parser.add_argument(
            "--kt-group-prefetch-policy",
            choices=["oracle", "static"],
            default=ServerArgs.kt_group_prefetch_policy,
            help="[experimental KT] Source of the per-layer expert plan.",
        )
        parser.add_argument(
            "--kt-group-oracle-required",
            action=argparse.BooleanOptionalAction,
            default=ServerArgs.kt_group_oracle_required,
            help="[experimental KT] Fail rather than silently use a static plan on oracle miss.",
        )
        parser.add_argument(
            "--kt-group-oracle-trace",
            type=str,
            default=ServerArgs.kt_group_oracle_trace,
            help="[experimental KT] KT-native routed-expert trace used by exact oracle plans.",
        )
        parser.add_argument(
            "--kt-group-oracle-prompt-identity-manifest",
            type=str,
            default=ServerArgs.kt_group_oracle_prompt_identity_manifest,
            help="[experimental KT] Prompt identity manifest paired with the oracle trace.",
        )
        parser.add_argument(
            "--kt-group-materialization",
            choices=["full", "delta"],
            default=ServerArgs.kt_group_materialization,
            help="[experimental KT] Materialize each group plan with full H2D or buffer-aware delta copies.",
        )
        parser.add_argument(
            "--kt-group-max-replacements",
            type=int,
            default=ServerArgs.kt_group_max_replacements,
            help="[experimental KT] Maximum logical replacements per layer between same-stage actions.",
        )
        parser.add_argument(
            "--kt-stage-ready-scheduler",
            action="store_true",
            default=ServerArgs.kt_stage_ready_scheduler,
            help="[experimental KT] Enable M20-B stage-ready action scheduling.",
        )
        parser.add_argument(
            "--kt-stage-policy",
            choices=["fifo", "min_delta", "cost_oracle"],
            default=ServerArgs.kt_stage_policy,
            help="[experimental KT] Stage-ready action selection policy.",
        )
        parser.add_argument(
            "--kt-stage-cohort-size",
            type=int,
            default=ServerArgs.kt_stage_cohort_size,
            help="[experimental KT] Maximum same-stage request chunks per action.",
        )
        parser.add_argument(
            "--kt-stage-candidate-window",
            type=int,
            default=ServerArgs.kt_stage_candidate_window,
            help="[experimental KT] Oldest same-stage chunks considered for an action.",
        )
        parser.add_argument(
            "--kt-stage-max-consecutive",
            type=int,
            default=ServerArgs.kt_stage_max_consecutive,
            help="[experimental KT] Maximum same-stage action streak when other stages are ready.",
        )
        parser.add_argument(
            "--kt-stage-max-wait-ms",
            type=float,
            default=ServerArgs.kt_stage_max_wait_ms,
            help="[experimental KT] Hard ready-queue wait bound in milliseconds; 0 disables it.",
        )
        parser.add_argument(
            "--kt-stage-max-inflight-chunks",
            type=int,
            default=ServerArgs.kt_stage_max_inflight_chunks,
            help="[experimental KT] Maximum suspended chunks owned by stage-ready queues.",
        )
        parser.add_argument(
            "--kt-stage-h2d-expert-ms",
            type=float,
            default=ServerArgs.kt_stage_h2d_expert_ms,
            help="[experimental KT] Diagnostic host-prepare plus H2D cost per cold expert.",
        )
        parser.add_argument(
            "--kt-stage-d2d-expert-ms",
            type=float,
            default=ServerArgs.kt_stage_d2d_expert_ms,
            help="[experimental KT] Diagnostic D2D cost per retained-source expert.",
        )
        parser.add_argument(
            "--kt-stage-route-entry-gain-ms",
            type=float,
            default=ServerArgs.kt_stage_route_entry_gain_ms,
            help="[experimental KT] Uncalibrated diagnostic critical-path gain per GPU-covered route entry.",
        )
        parser.add_argument(
            "--kt-stage-copy-contention-ms-per-expert",
            type=float,
            default=ServerArgs.kt_stage_copy_contention_ms_per_expert,
            help="[experimental KT] Additional copy-contention penalty per copied expert.",
        )
        parser.add_argument(
            "--kt-stage-eviction-route-weight",
            type=float,
            default=ServerArgs.kt_stage_eviction_route_weight,
            help="[experimental KT] Finite-window future-route eviction penalty weight.",
        )
        parser.add_argument(
            "--kt-stage-queue-penalty-ms-per-s",
            type=float,
            default=ServerArgs.kt_stage_queue_penalty_ms_per_s,
            help="[experimental KT] Cost penalty per second waited by skipped older chunks.",
        )
        parser.add_argument(
            "--kt-stage-min-gain-ms",
            type=float,
            default=ServerArgs.kt_stage_min_gain_ms,
            help="[experimental KT] Minimum predicted net gain required for cost-aware reordering.",
        )
        parser.add_argument(
            "--kt-stage-confidence-threshold",
            type=float,
            default=ServerArgs.kt_stage_confidence_threshold,
            help="[experimental KT] Minimum complete oracle/predictor demand confidence.",
        )
        parser.add_argument(
            "--kt-action-trace-path",
            type=str,
            default=ServerArgs.kt_action_trace_path,
            help="[experimental KT] Write the hash-chained M20-B action trace to this path.",
        )
        parser.add_argument(
            "--kt-action-replay-path",
            type=str,
            default=ServerArgs.kt_action_replay_path,
            help="[experimental KT] Replay an existing M20-B action trace exactly.",
        )
        parser.add_argument(
