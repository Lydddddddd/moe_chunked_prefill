...
from sglang.srt.layers.moe.kt_group_expert_buffer import (
    KTGroupBufferConfig,
...
_KT_GROUP_EXPERT_MANAGER: Optional[KTGroupExpertBufferManager] = None
...
class RuntimeOraclePrefetchProvider:
    ...
    def __init__(
        ...
        trace_path: str,
        *,
        prompt_identity_manifest_path: Optional[str] = None,
        require_prompt_identity: Optional[bool] = None,
    ):
        ...
        env_require_identity = os.environ.get(
            "SGLANG_KT_RUNTIME_ORACLE_PREFETCH_REQUIRE_PROMPT_IDENTITY", "0"
        ).strip().lower() in {"1", "true", "yes", "on"}
        self.require_prompt_identity = (
            env_require_identity
            if require_prompt_identity is None
            else bool(require_prompt_identity)
        )
        self.prompt_identity_manifest_path = (
            prompt_identity_manifest_path
            if prompt_identity_manifest_path is not None
            else os.environ.get(
    ...
    def ranked_experts_for_segments(
        self,
        ...
        *,
        ...
        """Rank experts for explicit prompt/token segments without mutable step state."""

        normalized = [dict(segment) for segment in segments]
        return self._ranked_experts_from_segments(
            segments=normalized,
            target_layer=int(target_layer),
            limit=int(limit),
        )

    def demand_for_batch(
        self,
        *,
        kt_metadata_list: Sequence[Mapping[str, Any]],
        prefix_lens: Sequence[int],
        extend_lens: Sequence[int],
        layer_indices: Iterable[int],
    ) -> Tuple[Dict[int, Dict[int, float]], Dict[int, float]]:
        """Return exact sparse route counts and completeness for one ready chunk.

        This API is deliberately stateless: stage scheduling can reorder chunks
        while route lookup remains keyed by prompt identity and absolute token
        span.  Confidence is the fraction of requested tokens with a valid trace
        slice for each layer; any identity or range miss therefore fails closed.
        """

        metadata = [dict(value) for value in kt_metadata_list]
        prefixes = [int(value) for value in prefix_lens]
        extensions = [int(value) for value in extend_lens]
        requested_tokens = sum(max(0, value) for value in extensions)
        context = {
            "kt_metadata_list": metadata,
            "extend_prefix_lens_cpu": prefixes,
            "extend_seq_lens_cpu": extensions,
        }
        segments = self._segments_from_context(context)
        demand_by_layer: Dict[int, Dict[int, float]] = {}
        confidence_by_layer: Dict[int, float] = {}
        import numpy as np

        for raw_layer_idx in layer_indices:
            layer_idx = int(raw_layer_idx)
            counts = np.zeros((self.num_experts,), dtype=np.int64)
            matched_tokens = 0
            for segment in segments:
                routed = self._lookup_routed(segment)
                if routed is None or not 0 <= layer_idx < int(routed.shape[1]):
                    continue
                token_start = max(0, int(segment.get("token_start", 0)))
                token_end = min(
                    int(segment.get("token_end", 0)), int(routed.shape[0])
                )
                if token_end <= token_start:
                    continue
                values = routed[token_start:token_end, layer_idx, :].reshape(-1)
                ...
                matched_tokens += token_end - token_start
            demand_by_layer[layer_idx] = {
                int(expert_id): float(value)
                for expert_id, value in enumerate(counts.tolist())
                if value > 0
            }
            confidence_by_layer[layer_idx] = (
                min(1.0, matched_tokens / requested_tokens)
                if requested_tokens > 0
                else 0.0
            )
        return demand_by_layer, confidence_by_layer
...
def get_runtime_oracle_prefetch_provider() -> Optional[RuntimeOraclePrefetchProvider]:
    ...
    try:
        ...
        pass
    group_oracle_enabled = bool(
        getattr(server_args, "kt_group_expert_buffer", False)
        and getattr(server_args, "kt_group_prefetch_policy", "") == "oracle"
    )
    env_oracle_enabled = (
        os.environ.get("SGLANG_KT_RUNTIME_ORACLE_PREFETCH", "0") == "1"
    )
    if not group_oracle_enabled and not env_oracle_enabled:
        return None
    trace_path = str(
        getattr(server_args, "kt_group_oracle_trace", None)
        or os.environ.get("SGLANG_KT_RUNTIME_ORACLE_PREFETCH_TRACE", "")
    )
    if not trace_path:
        logger.warning(
    ...
    identity_manifest = str(
        getattr(server_args, "kt_group_oracle_prompt_identity_manifest", None)
        or os.environ.get(
            "SGLANG_KT_RUNTIME_ORACLE_PREFETCH_PROMPT_IDENTITY_MANIFEST", ""
        )
    )
    try:
        _KT_RUNTIME_ORACLE_PREFETCH = RuntimeOraclePrefetchProvider(
            trace_path,
            prompt_identity_manifest_path=identity_manifest,
            require_prompt_identity=(
                True
                if group_oracle_enabled and identity_manifest
                else None
            ),
        )
        return _KT_RUNTIME_ORACLE_PREFETCH
...
class KTConfig:
    ...
    kt_group_expert_buffer: bool = False
    kt_group_size: int = 2
    kt_slots_per_layer: int = 4
    kt_group_buffer_count: int = 2
    kt_group_load_mode: str = "sync"
    kt_group_miss_policy: str = "block"
    kt_group_prefetch_policy: str = "oracle"
    kt_group_oracle_required: bool = True
    kt_group_materialization: str = "full"
    kt_group_max_replacements: Optional[int] = None


def _get_or_create_kt_group_manager(
    kt_config: KTConfig, *, num_experts: int
) -> KTGroupExpertBufferManager:
    global _KT_GROUP_EXPERT_MANAGER
    if not kt_config.kt_group_expert_buffer:
        raise RuntimeError("KT group manager requested while group mode is disabled")
    config = KTGroupBufferConfig(
        num_layers=int(kt_config.num_layers or 0),
        num_experts=int(num_experts),
        group_size=int(kt_config.kt_group_size),
        slots_per_layer=int(kt_config.kt_slots_per_layer),
        buffer_count=int(kt_config.kt_group_buffer_count),
        load_mode=str(kt_config.kt_group_load_mode),
        miss_policy=str(kt_config.kt_group_miss_policy),
        prefetch_policy=str(kt_config.kt_group_prefetch_policy),
        oracle_required=bool(kt_config.kt_group_oracle_required),
        require_bf16=True,
        materialization=str(kt_config.kt_group_materialization),
        max_replacements=kt_config.kt_group_max_replacements,
    )
    if _KT_GROUP_EXPERT_MANAGER is None:
        _KT_GROUP_EXPERT_MANAGER = KTGroupExpertBufferManager(config)
    elif _KT_GROUP_EXPERT_MANAGER.config != config:
        raise RuntimeError(
            "all KT group-buffer layers must use one identical configuration: "
            f"existing={_KT_GROUP_EXPERT_MANAGER.config} requested={config}"
        )
    return _KT_GROUP_EXPERT_MANAGER


def get_kt_group_expert_buffer_manager() -> Optional[KTGroupExpertBufferManager]:
    return _KT_GROUP_EXPERT_MANAGER


def kt_group_runtime_enabled() -> bool:
    return _KT_GROUP_EXPERT_MANAGER is not None


def begin_kt_group_forward(num_tokens: int) -> KTGroupExpertBufferManager:
    manager = _KT_GROUP_EXPERT_MANAGER
    if manager is None:
        raise RuntimeError("KT group-buffer runtime is not initialized")
    loader = get_runtime_hf_expert_loader()
    if loader is None:
        raise RuntimeError("KT group-buffer runtime requires the HF expert loader")
    provider = (
        get_runtime_oracle_prefetch_provider()
        if manager.config.prefetch_policy == "oracle"
        else None
    )
    manager.begin_step(num_tokens=int(num_tokens), loader=loader, provider=provider)
    return manager


def begin_kt_group_action(
    group_id: int,
    num_tokens: int,
    ...
    *,
...
) -> Tuple[KTGroupExpertBufferManager, Dict[int, Tuple[int, ...]]]:
    manager = _KT_GROUP_EXPERT_MANAGER
    if manager is None:
        raise RuntimeError("KT group-buffer runtime is not initialized")
    loader = get_runtime_hf_expert_loader()
    ...
    frozen = manager.begin_action(
        ...
        logical_source_buffer_id=logical_source_buffer_id,
        plan_hash=plan_hash,
    )
    return manager, frozen


def prefetch_kt_group_action(action_payload: Dict[str, Any]) -> bool:
    manager = _KT_GROUP_EXPERT_MANAGER
    if manager is None:
        raise RuntimeError("KT group-buffer runtime is not initialized")
    return manager.prefetch_action(action_payload)


def activate_kt_group(group_id: int) -> str:
    manager = _KT_GROUP_EXPERT_MANAGER
    if manager is None:
        raise RuntimeError("KT group-buffer runtime is not initialized")
    return manager.activate_group(int(group_id))


def finish_kt_group(group_id: int) -> None:
    manager = _KT_GROUP_EXPERT_MANAGER
    if manager is None:
        raise RuntimeError("KT group-buffer runtime is not initialized")
    manager.finish_group(int(group_id))


def end_kt_group_forward() -> Dict[str, Any]:
    manager = _KT_GROUP_EXPERT_MANAGER
    if manager is None:
        raise RuntimeError("KT group-buffer runtime is not initialized")
    return manager.end_step()


def end_kt_group_action(group_id: int) -> Dict[str, Any]:
    manager = _KT_GROUP_EXPERT_MANAGER
    if manager is None:
        raise RuntimeError("KT group-buffer runtime is not initialized")
    return manager.end_action(int(group_id))


def abort_kt_group_forward(reason: str) -> None:
    manager = _KT_GROUP_EXPERT_MANAGER
    if manager is not None and (
        manager.step_active or manager.has_pending_action
    ):
        manager.abort_step(str(reason))
...
def _generate_group_mode_initial_masks(
    *,
    strategy: str,
    activation_freq: Optional[torch.Tensor],
    num_layers: int,
    num_experts: int,
    slots_per_layer: int,
    first_k_dense_replace: int,
    ...
    """Create exactly ``slots_per_layer`` initial experts for every MoE layer.

    The legacy placement helpers consume a *global* expert budget.  Both group
    mode and the ``--kt-num-gpu-experts`` static contract need a per-layer
    logical width.  Reusing the global allocator makes a request for two
    experts per layer concentrate all slots in a subset of layers.
    """
    ...
    for layer_idx in range(num_layers):
        ...
        if not is_moe:
            masks[layer_idx, :] = True
            continue
        if strategy == "frequency" and activation_freq is not None:
            selected = torch.topk(
                activation_freq[layer_idx], k=slots_per_layer, largest=True
            ).indices
        elif strategy == "random":
            selected = torch.randperm(
                num_experts, generator=generator, device="cpu"
            )[:slots_per_layer]
        else:
            # uniform and front-loading have no meaningful cross-layer budget
            # under group mode, so select a stable per-layer prefix.
            selected = torch.arange(slots_per_layer, device="cpu")
        masks[layer_idx, selected] = True
    return masks


def _fixed_slots_per_layer_from_server_args(
    server_args: "ServerArgs", *, group_mode: bool
) -> Optional[int]:
    """Return the fixed per-layer width implied by the selected KT contract."""

    if group_mode:
        return int(server_args.kt_slots_per_layer)
    if (
        server_args.kt_gpu_experts_ratio is None
        and server_args.kt_num_gpu_experts is not None
    ):
        return int(server_args.kt_num_gpu_experts)
    return None
...
def _init_kt_gpu_experts_masks(server_args: "ServerArgs") -> Optional[torch.Tensor]:
    ...
    # Determine num_gpu_experts (total across all layers). Group-buffer mode
    # uses slots_per_layer as the logical placement width while physical
    # allocation is owned separately by KTGroupExpertBufferManager.
    if getattr(server_args, "kt_group_expert_buffer", False):
        slots_per_layer = int(server_args.kt_slots_per_layer)
        if num_moe_layers != num_layers:
            raise ValueError(
                "M20-A group buffers currently require every decoder layer to "
                f"be MoE, got {num_moe_layers} MoE layers out of {num_layers}"
            )
        if not 1 <= slots_per_layer <= num_experts:
            raise ValueError(
                f"kt_slots_per_layer must be in [1, {num_experts}], "
                f"got {slots_per_layer}"
            )
        if not 1 <= int(server_args.kt_group_size) <= num_layers:
            raise ValueError(
                f"kt_group_size must be in [1, {num_layers}], "
                f"got {server_args.kt_group_size}"
            )
        num_gpu_experts = slots_per_layer * num_moe_layers
        logger.warning(
            "KT group expert buffer placement: logical=%d layers x %d experts, "
            "physical=%d buffers x %d group layers x %d slots = %d slots",
            num_moe_layers,
            slots_per_layer,
            int(server_args.kt_group_buffer_count),
            int(server_args.kt_group_size),
            slots_per_layer,
            int(server_args.kt_group_buffer_count)
            * int(server_args.kt_group_size)
            * slots_per_layer,
    ...
    group_mode = bool(getattr(server_args, "kt_group_expert_buffer", False))
    # ``--kt-num-gpu-experts`` is documented and logged as a per-layer
    # quantity.  The historical allocators consume a global budget instead,
    # which can produce e.g. four experts in early layers and none in later
    # layers for a request of two per layer.  Preserve ratio mode as global,
    # but make the explicit count contract fixed-width for static and group
    # paths alike so their placements are comparable.
    fixed_slots_per_layer = _fixed_slots_per_layer_from_server_args(
        server_args, group_mode=group_mode
    )
    ...
    if strategy == "frequency":
        ...
        # Group mode has a fixed logical width per layer. Do not feed its
        # aggregate logical count into the legacy global-budget allocator.
        if tp_rank == 0:
            if fixed_slots_per_layer is not None:
                masks = _generate_group_mode_initial_masks(
                    strategy=strategy,
                    activation_freq=activation_freq,
                    num_layers=num_layers,
                    num_experts=num_experts,
                    slots_per_layer=fixed_slots_per_layer,
...
def create_kt_config_from_server_args(
    ...
    _packed_chunk_capacity = max(1, _multi_chunk_batch_size)
    if bool(getattr(server_args, "kt_stage_ready_scheduler", False)):
        _packed_chunk_capacity = max(
            _packed_chunk_capacity,
            int(getattr(server_args, "kt_stage_cohort_size", 1)),
        )
    if (
        effective_chunked_prefill_size is not None
        and effective_chunked_prefill_size > 0
        and _packed_chunk_capacity > 1
    ):
        effective_chunked_prefill_size *= _packed_chunk_capacity
    ...
    return KTConfig(
        ...
        kt_group_expert_buffer=bool(
            getattr(server_args, "kt_group_expert_buffer", False)
        ),
        kt_group_size=int(getattr(server_args, "kt_group_size", 2)),
        kt_slots_per_layer=int(getattr(server_args, "kt_slots_per_layer", 4)),
        kt_group_buffer_count=int(
            getattr(server_args, "kt_group_buffer_count", 2)
        ),
        kt_group_load_mode=str(
            getattr(server_args, "kt_group_load_mode", "sync")
        ),
        kt_group_miss_policy=str(
            getattr(server_args, "kt_group_miss_policy", "block")
        ),
        kt_group_prefetch_policy=str(
            getattr(server_args, "kt_group_prefetch_policy", "oracle")
        ),
        kt_group_oracle_required=bool(
            getattr(server_args, "kt_group_oracle_required", True)
        ),
        kt_group_materialization=str(
            getattr(server_args, "kt_group_materialization", "full")
        ),
        kt_group_max_replacements=getattr(
            server_args, "kt_group_max_replacements", None
        ),
    )
...
class RuntimeHFExpertWeightLoader:
    ...
    @property
    def group_async_host_memory_enabled(self) -> bool:
        """Whether group H2D sources are configured as page-locked memory."""

        return bool(self._pin_cpu_tensors or self._register_cpu_tensors)
    ...
    def prepare_expert_for_group_slot(
        ...
        *,
        ...
        """Prepare one expert on the host without enqueueing a CUDA copy.

        CUDA timing for the group buffer must start after safetensors lookup,
        cache/future waits, and any layout normalization.  Keeping this phase
        separate prevents copy-stream idle time from being reported as H2D.
        """
        ...
        def _fit(src: torch.Tensor, dst: torch.Tensor) -> torch.Tensor:
            if tuple(src.shape) == tuple(dst.shape):
                return src
            if src.dim() == 2 and tuple(src.transpose(0, 1).shape) == tuple(dst.shape):
                return src.transpose(0, 1).contiguous()
            raise ValueError(
                f"cannot copy expert tensor {tuple(src.shape)} into "
                f"group buffer view {tuple(dst.shape)}"
            )

        if w13_dst.dim() < 2 or int(w13_dst.shape[0]) % 2 != 0:
            raise ValueError(f"invalid BF16 w13 group slot shape: {tuple(w13_dst.shape)}")
        shard = int(w13_dst.shape[0] // 2)
        gate = _fit(gate, w13_dst[:shard])
        up = _fit(up, w13_dst[shard:])
        down = _fit(down, w2_dst)

        key = (int(layer_idx), int(logical_expert_id))
        with self._host_registration_lock:
            host_registered = key in self._host_registered_keys
        async_h2d_capable = host_registered or all(
            bool(tensor.is_pinned()) for tensor in (gate, up, down)
        )
        ...
        return {
            ...
            "tensors": (gate, up, down),
            "keepalive": [gate, up, down],
            "async_h2d_capable": async_h2d_capable,
        }
    ...
    def enqueue_prepared_group_expert(
        ...
        *,
        ...
        """Copy a host-prepared expert into manager-owned destination views."""
        ...
        shard = int(w13_dst.shape[0] // 2)
        if (
            tuple(gate.shape) != tuple(w13_dst[:shard].shape)
            or tuple(up.shape) != tuple(w13_dst[shard:].shape)
            or tuple(down.shape) != tuple(w2_dst.shape)
        ):
            raise ValueError("prepared expert shape changed before H2D enqueue")

        if stream is None:
            w13_dst[:shard].copy_(gate)
            w13_dst[shard:].copy_(up)
            w2_dst.copy_(down)
        else:
            with torch.cuda.stream(stream):
                w13_dst[:shard].copy_(gate, non_blocking=True)
                w13_dst[shard:].copy_(up, non_blocking=True)
                w2_dst.copy_(down, non_blocking=True)

        return {
            "source": prepared.get("source", "unknown"),
            "prefetch_wait_ms": float(prepared.get("prefetch_wait_ms", 0.0)),
            "h2d_bytes": int(prepared.get("h2d_bytes", 0)),
            "keepalive": [gate, up, down],
            "async_h2d_capable": bool(
                prepared.get("async_h2d_capable", False)
            ),
        }

    def copy_expert_to_group_slot(
        self,
        *,
        layer_idx: int,
        logical_expert_id: int,
        ...
        """Compatibility wrapper for callers that do not need split timing."""

        prepared = self.prepare_expert_for_group_slot(
            layer_idx=layer_idx,
            logical_expert_id=logical_expert_id,
            w13_dst=w13_dst,
            w2_dst=w2_dst,
        )
        return self.enqueue_prepared_group_expert(
            prepared=prepared,
            w13_dst=w13_dst,
            w2_dst=w2_dst,
            stream=stream,
...
class KTEPWrapperMethod(FusedMoEMethodBase):
    ...
    def __init__(
        ...
        self._kt_group_expert_buffer_enabled = bool(
            kt_config.kt_group_expert_buffer
        )
        self._kt_group_manager: Optional[KTGroupExpertBufferManager] = None
        self._kt_group_cpu_fallback = False
        if (
            self._kt_group_expert_buffer_enabled
            and kt_config.kt_enable_dynamic_expert_update
        ):
            raise ValueError(
                "KT group expert buffers are mutually exclusive with the M19 "
                "per-layer dynamic expert update path"
            )
    ...
    def create_weights(
        ...
        **extra_weight_attrs,
        ...
        if self._kt_group_expert_buffer_enabled:
            self._kt_group_manager = _get_or_create_kt_group_manager(
                self.kt_config, num_experts=num_experts
            )
            self._kt_group_manager.register_layer(
                int(self.kt_config.layer_idx), layer, self
            )
    ...
    def apply(
        ...
        if self._kt_group_expert_buffer_enabled:
            if self._kt_group_manager is None:
                raise RuntimeError("KT group-buffer wrapper has no manager")
            self._kt_group_manager.assert_layer_ready_for_apply(
                int(self.kt_config.layer_idx), layer
            )
        _kt_timing = (
        ...
        if (
            ...
            or self._kt_group_cpu_fallback
        ...
        if _kt_t_apply_start is not None:
            ...
            _log_interval = max(
                ...
                int(
                    os.environ.get(
                        "SGLANG_KT_HYBRID_TIMING_LOG_INTERVAL", "16"
                    )
                ),
            )
            if _log_interval == 1 or _step <= 16 or _step % _log_interval == 0:
                logger.info(
